...
note $doc.perl;
  ...
  # ==[ Private Stuff ]=========================================================
  #-----------------------------------------------------------------------------
  method !get-data ( Str:D $content-key --> Str ) {

    my Str $content;
    for @$=pod -> $pd {
      if $pd ~~ Pod::Block::Comment and       # comment block
         $pd.config<comment>:exists and       # must have comment key
         $pd.config{$content-key}:exists and  # and the selection key
         !$pd.config<comment> {               # comment must be False
...
=begin comment :!comment :linux-mimetypes
